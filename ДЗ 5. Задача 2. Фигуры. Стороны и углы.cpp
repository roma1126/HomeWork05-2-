...
public:
	...
	void print_info()
	{
		std::cout << name << std::endl << get_sides() << get_corners() << "" << " " << " " << "\n";
	}
...
protected:
	...
	Figure(int sidesNumber, std::string name)
	{ 
...
protected:
	Triangle(int sidesNumber, std::string name)
	{
		this->name = name;
		this->sidesNumber = sidesNumber;
	}
	Triangle(int a, int b, int c, int A, int B, int C, int sidesNumber, std::string name)
	{
		{
...
{
public:
	isoscelesTriangle(int a, int c, int A, int B) :Triangle(a, c, a, A, B, A, 3, "Равнобедренный треугольник: ")
	{
...
protected:
	Quadrangle(int a, int b, int c, int d, int A, int B, int C, int D, int sidesNumber, std::string name)
	{
